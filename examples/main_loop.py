...
print('Initializing induction factor optimization loop')
...
while True:
    ...
    # print(' \n local angles of attack computed')
    ...
    angles_df['axial_induction_new'] = fn.update_axial(angles_df)
    ...
    angles_df['tangential_induction_new'] = fn.update_tangential(angles_df)
    ...
    if (np.abs(angles_df['axial_induction_new'] - angles_df['axial_induction']) < TOLERANCE).all() and \
       (np.abs(angles_df['tangential_induction_new'] - angles_df['tangential_induction']) < TOLERANCE).all():
        print('Convergence reached at iteration:', iteration_counter)
        break
    else:
        # Update the values of a and a' for the next iteration
        angles_df['axial_induction'] = angles_df['axial_induction_new']
        angles_df['tangential_induction'] = angles_df['tangential_induction_new']
        # print('Convergence not reached, continuing iterations...')
...
print(f"Power coefficient (CP): {power_coeff:.2f}")
