...
def main(POWER_CURVE_INDICE):
    ...
    #local angle of attack range
    local_angle_of_attack_range = f"{angles_df['local_angle_of_attack_deg'].min():.2f} ; {angles_df['local_angle_of_attack_deg'].max():.2f}"
    
    return local_angle_of_attack_range

loc_list = []
for i in range(len(power_curve_df)):
    # print(' \n running main.py for wind speed:', power_curve_df['wind_speed'].iloc[i])
    loc_list.append(main(i))
